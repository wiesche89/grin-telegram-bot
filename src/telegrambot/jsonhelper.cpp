...
/**
 * @brief JsonHelper::jsonPathGetImpl
 * @param data
 * @param path
 * @param showWarnings
 * @return
...
QVariant JsonHelper::jsonPathGetImpl(QJsonValue data, QString path, bool showWarnings)
{
    ...
    while (!splittedPath.isEmpty()) {
        ...
        if (data.type() == QJsonValue::Array) {
            ...
            if (!isInt) {
                if (showWarnings) {
                    qWarning("JsonHelper::jsonPath: pos: %s -> QJsonArray path element is non numeric", qPrintable(processed.join(".")));
                }
                intPath = -1;
        ...
        // handle all other data types
            ...
            if (showWarnings && !splittedPath.isEmpty()) {
                qWarning("JsonHelper::jsonPath: pos: %s -> Reached data end, but still path data available (%s)",
                         qPrintable(processed.join(".")), qPrintable(splittedPath.join(".")));
            }
