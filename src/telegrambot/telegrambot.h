...
public:
    ...
    static inline TelegramBotKeyboardButtonRequest constructTextButton(QString text, bool requestContact = false,
                                                                       bool requestLocation = false)
    {
        return TelegramBotKeyboardButtonRequest{ text, QString(), QString(), QString(), QString(), requestContact, requestLocation };
    }

    static inline TelegramBotKeyboardButtonRequest constructInlineButton(QString text, QString callbackData,
                                                                         QString url = QString(),
                                                                         QString inlineQueryData = QString(),
                                                                         QString inlineQueryDataCurrentChat = QString())
    {
        return TelegramBotKeyboardButtonRequest{ text, url, callbackData, inlineQueryData, inlineQueryDataCurrentChat, false, false };
    }

    static TelegramKeyboardRequest constructInlineMenu(QList<QString> menu, QString dataPattern, int page, int columns, int limit,
                                                       QString lastPage = "");
    ...
    void answerCallbackQuery(QString callbackQueryId,
                             QString text = QString(), bool showAlert = false, int cacheTime = 0,
                             QString url = QString(), bool *response = 0);

    // Message Functions
    void sendMessage(QVariant chatId, QString text, int replyToMessageId = 0, TelegramFlags flags = TelegramFlags::NoFlag,
                     TelegramKeyboardRequest keyboard = TelegramKeyboardRequest(), TelegramBotMessage *response = 0);
    void editMessageText(QVariant chatId, QVariant messageId, QString text, TelegramFlags flags = TelegramFlags::NoFlag,
                         TelegramKeyboardRequest keyboard = TelegramKeyboardRequest(), bool *response = 0);
    void editMessageCaption(QVariant chatId, QVariant messageId,
                            QString caption = QString(), TelegramKeyboardRequest keyboard = TelegramKeyboardRequest(), bool *response = 0);
    void editMessageReplyMarkup(QVariant chatId, QVariant messageId,
                                TelegramKeyboardRequest keyboard = TelegramKeyboardRequest(), bool *response = 0);
    void forwardMessage(QVariant targetChatId, QVariant fromChatId, qint32 fromMessageId, TelegramFlags flags = TelegramFlags::NoFlag,
                        TelegramBotMessage *response = 0);
    void deleteMessage(QVariant chatId, qint32 messageId, bool *response = 0);

    // Content Functions
    void sendPhoto(QVariant chatId, QVariant photo,
    ...
    void sendSticker(QVariant chatId, QVariant sticker, int replyToMessageId = 0, TelegramFlags flags = TelegramFlags::NoFlag,
                     TelegramKeyboardRequest keyboard = TelegramKeyboardRequest(), TelegramBotMessage *response = 0);
    void sendVideo(QVariant chatId, QVariant video,
                   QString caption = QString(), int duration = -1, int width = -1, int height = -1, int replyToMessageId = 0, TelegramFlags flags = TelegramFlags::NoFlag,
                   TelegramKeyboardRequest keyboard = TelegramKeyboardRequest(), TelegramBotMessage *response = 0);
    void sendVoice(QVariant chatId, QVariant voice,
                   QString caption = QString(), int duration = -1, int replyToMessageId = 0, TelegramFlags flags = TelegramFlags::NoFlag,
                   TelegramKeyboardRequest keyboard = TelegramKeyboardRequest(), TelegramBotMessage *response = 0);
    void sendVideoNote(QVariant chatId, QVariant videoNote, int length = -1, int duration = -1, int replyToMessageId = 0,
                       TelegramFlags flags = TelegramFlags::NoFlag,
                       TelegramKeyboardRequest keyboard = TelegramKeyboardRequest(), TelegramBotMessage *response = 0);
    void sendLocation(QVariant chatId, double latitude, double longitude, int replyToMessageId = 0,
                      TelegramFlags flags = TelegramFlags::NoFlag,
                      TelegramKeyboardRequest keyboard = TelegramKeyboardRequest(), TelegramBotMessage *response = 0);
    void sendVenue(QVariant chatId, double latitude, double longitude, QString title, QString address,
                   QString foursquareId = QString(), int replyToMessageId = 0, TelegramFlags flags = TelegramFlags::NoFlag,
                   TelegramKeyboardRequest keyboard = TelegramKeyboardRequest(), TelegramBotMessage *response = 0);
    void sendContact(QVariant chatId, QString phoneNumber, QString firstName,
                     QString lastName = QString(), int replyToMessageId = 0, TelegramFlags flags = TelegramFlags::NoFlag,
                     TelegramKeyboardRequest keyboard = TelegramKeyboardRequest(), TelegramBotMessage *response = 0);

    // Message Puller
    void startMessagePulling(uint timeout = 10, uint limit = 100, TelegramPollMessageTypes messageTypes = TelegramPollMessageTypes::All,
                             long offset = 0);
    ...
    void messageRouterRegister(QString startWith, QDelegate<bool(TelegramBotUpdate)> delegate,
                               TelegramBotMessageType type = TelegramBotMessageType::All);
...
private:
    // call Api Helpers
    template<typename T>
    typename std::enable_if<std::is_base_of<TelegramBotObject, T>::value>::type callApiTemplate(QString method,
                                                                                                QUrlQuery params = QUrlQuery(), T *response = 0, QHttpMultiPart *multiPart = 0);
    template<typename T>
    typename std::enable_if<!std::is_base_of<TelegramBotObject, T>::value>::type callApiTemplate(QString method,
                                                                                                 QUrlQuery params = QUrlQuery(), T *response = 0, QHttpMultiPart *multiPart = 0);
