...
public:
    ...
    {
        QJsonObject object
            = showWarnings ? JsonHelper::jsonPathGet(data, path).toJsonObject() : JsonHelper::jsonPathGetSilent(data, path).toJsonObject();
        if (object.isEmpty()) {
            return false;
        }
    ...
    {
        QJsonArray array
            = showWarnings ? JsonHelper::jsonPathGet(data, path).toJsonArray() : JsonHelper::jsonPathGetSilent(data, path).toJsonArray();
        if (array.isEmpty()) {
            return false;
        }
